...
import ch.ethz.matsim.mode_choice.mnl.ModeChoiceMNL.Mode;

public class FFModeChoiceMNL implements ModeChoiceModel {
	...
	final private MembershipContainer membership;

	
	final private Mode modelMode;

	public FFModeChoiceMNL(Random random, ChainAlternatives tripChainAlternatives, Network network, Mode modelMode, double chainPrior, MembershipContainer membership) {
	...
	public FFModeChoiceMNL(Random random, ChainAlternatives tripChainAlternatives, Network network, Mode modelMode, MembershipContainer membership) {
		this(random, tripChainAlternatives, network, modelMode, defaultChainPrior,membership);
	}
	...
	public List<String> chooseModes(Plan plan) {
		...
		List<String> individualNonChainModes = nonChainModes;

		if (!membership.getPerPersonMemberships().containsKey(plan.getPerson().getId())){
			individualNonChainModes = nonChainModes.stream().filter(m -> !m.equals("freefloating")).collect(Collectors.toList());

		}

		List<List<String>> feasibleTripChains = chainAlternatives.getTripChainAlternatives(plan, individualChainModes,
				individualNonChainModes);
